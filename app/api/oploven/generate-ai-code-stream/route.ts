...
import type { SandboxState } from '@/oploven_types/sandbox';
import { selectFilesForEdit, getFileContents, formatFilesForAI } from '@/oploven_lib/context-selector';
import { executeSearchPlan, formatSearchResultsForAI, selectTargetFile } from '@/oploven_lib/file-search-executor';
import { FileManifest } from '@/oploven_types/file-manifest';
import type { ConversationState, ConversationMessage, ConversationEdit } from '@/oploven_types/conversation';
      ...
      try {
        ...
        if (isEdit) {
          ...
          if (manifest) {
            ...
            try {
              const intentResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/oploven/analyze-edit-intent`, {
                method: 'POST',
          ...
          if (editContext) {
            ...
            if (global.activeSandbox) {
              ...
              try {
                ...
                const filesResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/oploven/get-sandbox-files`, {
                  method: 'GET',
                  headers: { 'Content-Type': 'application/json' }
                });
