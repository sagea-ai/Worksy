...
import { sandboxManager } from '@/oploven_lib/sandbox/sandbox-manager';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!provider) {
      ...
      try {
        const { SandboxFactory } = await import('@/oploven_lib/sandbox/factory');
      ...
      try {
        ...
        if (uniquePackages.length > 0) {
          ...
          try {
            ...
            const apiUrl = `${protocol}://${host}/api/oploven/install-packages`;
