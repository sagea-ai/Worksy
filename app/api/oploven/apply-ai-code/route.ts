...
export async function POST(request: NextRequest) {
  try {
    ...
    if (uniquePackages.length > 0) {
      ...
      try {
        const installResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/oploven/install-packages`, {
          method: 'POST',
      ...
      try {
        ...
        const packageResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/oploven/detect-and-install-packages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (results.packagesInstalled.length > 0) {
          ...
          try {
            ...
            const restartResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/oploven/restart-vite`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' }
            });
    ...
    if (missingImports.length > 0) {
      ...
      try {
        ...
        const autoCompleteResponse = await fetch(
          `${request.nextUrl.origin}/api/oploven/auto-complete-components`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
